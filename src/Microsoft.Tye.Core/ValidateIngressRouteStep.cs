...
{
    public sealed class ValidateIngressRouteStep : ApplicationExecutor.IngressRouteStep
    {
        public override string DisplayText => "Validating IngressRoute...";
        ...
        public override async Task ExecuteAsync(OutputContext output, ApplicationBuilder application, IngressRouteBuilder ingress)
        {
            ...
            {
                throw new CommandException($"Cannot validate ingressRoute because kubectl is not installed.");
            }

            if (!await KubectlDetector.IsKubectlConnectedToClusterAsync(output))
            {
                throw new CommandException($"Cannot validate ingressRoute because kubectl is not connected to a cluster.");
            }
